...
from .data.data_utils import create_prompt_dataset,DataCollatorReward, DataCollatorReward_modify
...
def make_reward_prompt_dataloader(tokenizer: transformers.PreTrainedTokenizer, data_args, engine, val_split=None) -> Dict:
    train_phase = 2
    train_dataset, eval_dataset = create_prompt_dataset(
        local_rank=data_args.local_rank, 
        data_path=[data_args.data_path], 
        data_split="2,2,4",
        output_path=data_args.data_output_path, 
        train_phase=train_phase, 
        seed=data_args.seed, 
        tokenizer=tokenizer,
        max_seq_len=data_args.max_seq_len) 
    # DataLoaders creation:
    data_collator = DataCollatorReward_modify(tokenizer, data_args.num_padding_at_beginning)
    g = torch.Generator()
    ...
    eval_sampler = DistributedSampler(eval_dataset,
                                      ...
                                      rank=engine.mpu.get_data_parallel_rank(),
                                       shuffle=False)
    # train_pipe_dataloader = engine.deepspeed_io(train_dataset, data_sampler=train_sampler, collate_fn=data_collator, pin_memory=False)
    # eval_pipe_dataloader = engine.deepspeed_io(eval_dataset, data_sampler=eval_sampler, collate_fn=data_collator, pin_memory=False)

    train_pipe_dataloader = DataLoader(train_dataset,
                            ...
                            drop_last=True,
                            generator=g,)
    eval_pipe_dataloader = DataLoader(eval_dataset,
                            collate_fn=data_collator,
                            num_workers=data_args.num_workers,
                            batch_size=data_args.batch_size,
    ...
    return (iter(deepspeed.utils.RepeatingLoader(train_pipe_dataloader)), iter(deepspeed.utils.RepeatingLoader(eval_pipe_dataloader)))
