...
try:
    from flash_attn import flash_attn_func, __version__, flash_attn_varlen_func
...
def apply_rotary_pos_emb2(q, k, cos, sin, position_ids):
    # The first two dimensions of cos and sin are always 1, so we can `squeeze` them.
    cos = cos.squeeze(1).squeeze(0)  # [seq_len, dim]
    sin = sin.squeeze(1).squeeze(0)  # [seq_len, dim]
...
class YiAttention2(YiAttention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        query_states, key_states = apply_rotary_pos_emb2(query_states, key_states, cos, sin, position_ids)
...
class YiDecoderLayer(nn.Module):
    def __init__(self, config: YiConfig):
        ...
        self.hidden_size = config.hidden_size
        self.self_attn = YiAttention2(config=config)
        self.mlp = YiMLP(
